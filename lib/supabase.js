...
export const updateChatTitle = async (chatId, newTitle) => {
  try {
    const { data, error } = await supabase.from("chats").update({ title: newTitle }).eq("id", chatId).select().single()
...
export const updateChatMessages = async (chatId, messages) => {
  try {
    const { data, error } = await supabase.from("chats").update({ messages }).eq("id", chatId).select().single()
